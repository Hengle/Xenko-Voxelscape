...
namespace Voxelscape.Utility.Common.Core.Collections
{
	...
	{
		/// <summary>
		/// Initializes a new instance of the <see cref="TupleStruct{T1,T2,T3,T4,T5,T6,T7,T8}"/> struct.
		/// </summary>
		...
		public static bool operator ==(
			TupleStruct<T1, T2, T3, T4, T5, T6, T7, T8> lhs, TupleStruct<T1, T2, T3, T4, T5, T6, T7, T8> rhs) =>
			lhs.Equals(rhs);
		...
		#region IEquatable<TupleStruct<T1, T2, T3, T4, T5, T6, T7, T8>> Members
		...
		public bool Equals(TupleStruct<T1, T2, T3, T4, T5, T6, T7, T8> other) =>
			...
			this.Item8.EqualsNullSafe(other.Item8);
		...
		public override int GetHashCode() => HashCode.Start
			.And(this.Item1).And(this.Item2).And(this.Item3).And(this.Item4).And(this.Item5).And(this.Item6).And(this.Item7).And(this.Item8);
