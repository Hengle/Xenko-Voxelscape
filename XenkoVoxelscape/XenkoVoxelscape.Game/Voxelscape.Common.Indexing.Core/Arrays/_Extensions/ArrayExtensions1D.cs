...
public static class ArrayExtensions1D
{
	...
	public static int GetLength<T>(this T[] array, Axis1D dimension)
	{
	...
	public static long GetLongLength<T>(this T[] array, Axis1D dimension)
	{
		Contracts.Requires.That(array != null);
	...
	public static int GetLowerBound<T>(this T[] array, Axis1D dimension)
	{
		Contracts.Requires.That(array != null);
	...
	/// </returns>
	public static int GetUpperBound<T>(this T[] array, Axis1D dimension)
	{
		Contracts.Requires.That(array != null);
	...
	public static Index1D GetDimensions<T>(this T[] array)
	{
		...
		return new Index1D(array.GetLength((int)Axis1D.X));
	}
	...
	public static Index1D GetLowerBounds<T>(this T[] array)
	{
		...
		return new Index1D(array.GetLowerBound((int)Axis1D.X));
	}
	...
	public static Index1D GetUpperBounds<T>(this T[] array)
	{
		...
		return new Index1D(array.GetUpperBound((int)Axis1D.X));
	}
	...
	public static IEnumerable<KeyValuePair<Index1D, T>> GetIndexValuePairs<T>(this T[] array)
	{
		Contracts.Requires.That(array != null);

		foreach (var index in Index.Range(array.GetLowerBounds(), array.GetDimensions()))
		{
			yield return new KeyValuePair<Index1D, T>(index, array[index.X]);
		}
	}
	...
	public static bool IsIndexValid<T>(this T[] array, Index1D index)
	{
		Contracts.Requires.That(array != null);
		...
		this TSource[] array, Converter<TSource, TResult> converter)
	{
		...
		{
			result[pair.Key.X] = converter(pair.Value);
		}
