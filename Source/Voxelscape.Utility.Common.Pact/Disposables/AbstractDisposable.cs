...
namespace Voxelscape.Utility.Common.Pact.Disposables
{
	...
	public abstract class AbstractDisposable : IVisiblyDisposable
	{
		...
		/// </summary>
		/// <remarks>
		/// The finalizer is only added in debug builds to ensure the <see cref="Dispose"/> method is called.
		/// It is removed from all other builds for performance and because the type should not handle any
		/// unmanaged resources.
		/// </remarks>
		[SuppressMessage("Microsoft.Design", "CA1063", Justification = "Better disposable pattern.")]
		~AbstractDisposable()
		{
			throw FinalizerRanException.CreateException(this);
		}
